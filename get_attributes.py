...
def process_file(src):
    ...
    for sheet in details['sheets']:
        ...
        # Rename the second column in this sheet to "FieldName"
        ...
        migration_status_col = details['fields'].get('migration_status_col', None)
        populated_col = details['fields'].get('populated_col', None)

        if migration_status_col is not None and populated_col is not None:
            filtered_df = df[
...
def main():
    ...
    combined_df[['sheet', 'FieldName', 'DataType', 'Maxlength', 'TargetEntity']].to_csv('attributes.csv', sep='|', index=False)
